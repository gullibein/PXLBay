...
for (let s = 0; s < 10; s++) {
  ...
  for (let d = 1; d <= 26; d++) {
    ...
    if (wb) { if(d===9&&s===0){require("fs").writeFileSync("/tmp/unwalled.json",JSON.stringify({msg:wb,w:ctx.MAP_W,h:ctx.MAP_H,tiles:Array.from(ctx.L.tiles),decor:ctx.L.decor,px:ctx.P.x,py:ctx.P.y}));} wallFail++; if (wallFail < 4) console.log('  L' + d + ' seed' + s + ': ' + wb); }
    const kb = ctx.keysOK();
